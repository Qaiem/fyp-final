...
//     // Allow multiple tasks to be in progress, todo, or complete
//     task.stage = stage.toLowerCase();
...
//     res.status(200).json({
//       status: true,
//       message: `Task stage updated to '${stage}' successfully.`,
//     });
//   } catch (error) {
...
const upload_screenshot = async (req, res) => {
  try {
    ...
    // Confirm body contains binary data
    if (!req.body || !req.body.length) {
      console.log("❌ No image data received");
      return res.status(400).json({ message: "No image data provided" });
    }

    // Log to verify what was received
    ...
    await task.save();
    console.log("✅ Screenshot saved successfully for:", taskId);

    res.status(200).json({ message: "Screenshot uploaded successfully" });
  } catch (error) {
    console.error("💥 Error in upload_screenshot:", error);
    res.status(500).json({ message: "Server error", error: error.message });
  }
};

// Get screenshots of a task
const getTaskScreenshots = async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      task: {
        _id: task._id,
        screenshots: task.screenshots, // send Buffer array as-is
      },
    });
  } catch (error) {
    console.error(error);
